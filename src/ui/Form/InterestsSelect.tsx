...
export async function fetchInterestsOptions(inputValue: string) {
    //console.log(props)
    return API.get<any>('/user/interest',   { params: { name: inputValue } }).then(
        (response) => {
...
export const InterestsSelect = ({ placeholder, object, ...rest }: any) => {
    ...
    useEffect(() => {
        fetchInterestsOptions(current).then(res => {
            if (res.length > 0)
                setOptions(res)
            else {
